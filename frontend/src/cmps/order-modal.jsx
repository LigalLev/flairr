            ...
            {orders.filter(order=>{return order.buyer._id === user._id}).map(order =>
                <li key={order._id} className="order-list-item">
                    ...
                    <div className="order-info-container">
                        <p className="order-gig-title">{order.gig.title}</p>
                        <div className="order-list-buyer">
                            <div className="order-list-buyer-name" >by {order.seller.fullname}</div>
                            <div className={`${order.status.toLowerCase()} status`}>{order.status}</div>
                        </div>
                    </div>
                </li>)}
        </ul>}
        {orders?.length < 1 && <div className="buyer-order-list-container"><svg viewBox="0 0 65 43"><g id="02H_XXS_Overview_empty_state_Admin" stroke="none"
            stroke-width="1" fill="none" fill-rule="evenodd"
            transform="translate(-127.000000, -235.000000)"><g
                id="Page-1-Copy" transform="translate(127.500000, 235.130859)" fill="#555555" stroke="#555555"><path
                    ...
                    <p className="empty-list-title">No Orders Yet</p>
                    <p className="empty-list">Use the search box to find the digital services you need</p>
                    </div>}
